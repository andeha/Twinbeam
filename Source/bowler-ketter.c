...
{
   INIT init₋with₋one₋collection = ^(void * uninited) { return init₋collection(sizeof()) };
   struct collection * list = jot(key₋copy,opaque,sizeof(struct collection),alloc,init₋with₋one₋collection);
   ...
   if (list->copy₋append₋items(sizeof(unicode₋shatter),&value₋text,list,alloc)) { return -1; }
   return 0;
...
int dissociate₋isolate(struct Unicodes key, int index, void ** opaque)
{
   ...
   if (node != ΨΛΩ) { unicode₋shatter key; struct collection * list;
     ...
     key = (unicode₋shatter)collection₋at(0,list);
     ...
     for (i=0; i<count; i+=1) {
       material = (unicode₋shatter)collection₋at(1+i,list);
...
int dissociate₋all(struct Unicodes key, void ** opaque)
{
   ...
   if (node != ΨΛΩ) { struct collection * list;
     unicode₋shatter key;
     list = (struct collection *)note->node;
     key = (unicode₋shatter)collection₋at(0,list);
...
{
   __uint128_t fineprint = FNV1b(4*tetras,key.unicodes);
   struct w₋node * node = impression₋seek(opaque,fineprint);
