...
#ifdef __mips__ // clz $a0, $v0,   ⃨  or clo.
...
{
    __builtin_int_t bytes = content.byteCount;
    __builtin_int_t i = beam;
    while (i < bytes) {
        SemanticPointer<void *> p = content.region->pointer(i + content.bytesOffset);
...
{
    INNER_DATA const unsigned char firstByteMark[7] = { 0x00, 0x00, 0xC0, 0xE0,
        0xF0, 0xF8, 0xFC };
    
    INNER_DATA const char32_t byteMask = 0xBF;
    INNER_DATA const char32_t byteMark = 0x80;
