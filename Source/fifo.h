...
struct fifo { __builtin_int_t E₋enfoiled, count=0, brk=0; __builtin_uint_t * 𝟷₋tile; 
 ...
 value on pic32mzda read from ADCFLTRx.FLTRDATA<15:0> 𝘪․𝘦 bit 15 - bit 0. */
   ...
   int n₋tile₋fifo₋pop(__builtin_int_t elems, void (^many₋𝟺kb₋etiolates)(__builtin_int_t tot, __builtin_int_t ﹟, void * bottom₋elem));
...
template <typename E> struct vector { fifo f; 
  int apriori₋init(unsigned expeditionary, 
    __builtin_int_t alloced₋count, __builtin_int_t max₋count)
  {
  ...
  int pop(__builtin_int_t elems, void (^many₋𝟺kb₋etiolates)(__builtin_int_t tot, __builtin_int_t ﹟, void * bottom₋elem)) 
  { return f.n₋tile₋fifo₋pop(elems,many₋𝟺kb₋etiolates); }
}; /* a․𝘬․a `n₋tile₋fifo`. */
