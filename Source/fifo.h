...
struct ᵒfᵗᵉⁿ𝟺kb₋embroidery { __builtin_int_t ﹟, w₋ord₋s; void **Kbtiles; };
/* ⬷ 𝘊․𝘧 'motsols', 'medsols' and 'haphazardly' also 'covert-lazily'. */
...
template <typename E> struct 𝟷₋tile₋fifo { fifo 🅠; 
  ...
  consteval __builtin_int_t log₂(__builtin_int_t 𝟸ⁿ) { return 𝟸ⁿ==8 ? 3 : (𝟸ⁿ==4 ? 2 : -1); }
  /* ⬷ Must produce a compile-time constant a․𝘬․a 'consteval'. */
  
  constexpr __builtin_int_t ceil(__builtin_int_t num, __builtin_int_t 𝟸ⁿ) { return Frame(num,𝟸ⁿ)>>log₂(𝟸ⁿ); }
