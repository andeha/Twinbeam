...
__builtin_int_t Byteoffset⁸𝟷ᵈ(__builtin_int_t byteNº, __builtin_int_t 𝛥bytes, 
 enum Sentinel wrap, __builtin_int_t totbytes);
__builtin_int_t ByteoffsetMips𝟷ᵈ(__builtin_int_t byteNº, __builtin_int_t 𝛥mips, 
 enum Sentinel wrap, __builtin_int_t totmips);
...
#pragma header enum { 𝟾, 𝟷𝟼, 𝟹𝟸, 𝟼𝟺, lo𝟼𝟺, hi𝟼𝟺, lo𝟷𝟸𝟾, hi𝟷𝟸𝟾, utf8 }

struct Bits { struct collection * scatter; /* uint8_t& operator[](__builtin_int_t idx); */ };
/* ⬷ a․𝘬․a 'Memoryregion'. */
EXT₋C uint8_t ⁸𝟷ᵈ(__builtin_int_t byteNº, __builtin_int_t 𝛥bytes, enum Sentinel 
 wrap, __builtin_int_t totbytes, uint8_t * towrite𝘖r𝖭𝖴𝖫𝖫);
EXT₋C uint32_t mips𝟷ᵈ(__builtin_int_t byteNº, __builtin_int_t 𝛥mips, enum Sentinel 
 wrap, __builtin_int_t totmips, uint32_t * towrite𝘖r𝖭𝖴𝖫𝖫);
...
EXT₋C int OptimisticSwap(__builtin_int_t * p₁, __builtin_int_t * p₂, enum Impediment it);
...
struct AnnotatedRegister { const char32̄_t * header; int regcnt; struct Bitfield * regs; uint32_t init; const char32̄_t * footnote; };
...
/*
 ...
 union UQ3232 { uint64_t bits; struct { uint32_t lo, hi; } parts; };
   ⬷ a․𝘬․a 'Ntp₋stomp', captures 0 to +4294967295.99999999976716935634613037109375.
 */
...
coro_t * coro_await(Coroutine coroutine);
