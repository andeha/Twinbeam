/*  Bounds.cpp | On 'interval' and pointer. 
 ...
 Compile with clang++ -I ../.. --std=c++2a -o Bounds '🥽⋆Bounds.cpp' */
...
struct Intervallic { /* A․𝘬․a `TransitiveIntervalPointer`. */
  ...
  enum { UnableToRelretrieve=10, UnableToKeep=11 };
...
#pragma mark - First a word-granola:
  ...
  __builtin_uint_t * aligned(uint8_t * /* byteaddress */ unaligned, short &lshbits) const {
    ...
    lshbits=~mask&(__builtin_uint_t)unaligned;
    return wordloc;
  };
  
  __builtin_uint_t word(uint8_t * /* byteaddress */ unaligned, void (^issue)(int nº)) const {
     /* virtuaddr vaddr=loc.pointer; int ᵚoffset=vaddr&0xfff; int long idxᵖᵗ=vaddr>>12; */
     static __builtin_uint_t dummy=0;
     if (unaligned < base || unaligned >= base + bʸtes) { issue(0); return dummy; }
...
#pragma mark - The overt simpification
...
#pragma mark - Nitty-gritty and the details
  ...
  /* enum Width { 𝟾, 𝟷𝟼, 𝟹𝟸, 𝟼𝟺, 𝟷𝟸𝟾lo, 𝟷𝟸𝟾hi };
  ...
  {  Delegate * d = delegate; static uint8_t dummy=0;
    ...
    if (toadd == -1) { issue(UnableToRelretrieve); }  /* *byteloc=byte; Or: */
...
{  int 🥈 bytes = 4096;
    ...
    Delegate delegate; Intervallic interval₁(buf,bytes,&delegate);
    auto hitch = ^{ printf("Writing outside bounds\n"); exit(1); };
    auto ʳᵈissue = ^(int nº) { printf("Reading outside bounds\n"); exit(2); };
    
    uint8_t * /* byteaddress */ wloc=100+(uint8_t *)buf; __builtin_uint_t word=0xcafecafe;
