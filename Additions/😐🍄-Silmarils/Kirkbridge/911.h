...
BITMASK (uint8_t) { 𝙰𝙲𝙲_𝚘𝚟𝚎𝚛𝚏𝚕𝚘𝚠 = 0b1<<3, 𝙰𝙲𝙲_𝚌𝚊𝚛𝚛𝚢ₒ = 0b1<<2, 𝙰𝙲𝙲_𝚌𝚊𝚛𝚛𝚢ᵢ =
  0b1<<1, 𝙰𝙲𝙲_𝚣𝚎𝚛𝚘 = 0b1<<0 };

union Register { __uint128_t u; __int128_t s; uint64_t octas[2]; uint32_t
  tetras[4]; uint16_t halfs[8]; uint8_t bytes[16]; };

struct Registerset { 𝚏𝚙, 𝚜𝚙; Register gprs[𝚊𝚌𝚌𝟶𝚕𝚘+1], 𝚏𝚕𝚊𝚐𝚜; };

struct µProcᵖ { uint64_t 𝚙𝚌; void (*irq)(); uint8_t set; Registerset regs[7]; };
...
extern void (^👉ᵢ)(int16_t r_offset, gpr r, gpr r₌);
extern void (^👉ᵢ)(gpr r, gpr r₌, int16_t r₌_offset);
...
extern void (^𝚛𝚎𝚌𝚒𝚙𝚛𝚘𝚌)(gpr r, gpr inv, 𝙶𝚛𝚊𝚗𝚞𝚕 g, gpr r₌);
