...
#pragma mark Coordinate Systems and Functionals
...
void Mercator⁻¹(simd_tᵦ 𝒑𝑥, simd_tᵦ 𝒑𝑦, simd_tᵦ 𝜃₀, double R₁₂, simd_tᵦ& 𝜃ᵒᵘᵗ, simd_tᵦ& 𝜑ᵒᵘᵗ); /* See --<Geologics.hpp> for a generalization of the parameter `R₁₂`. */
FOSSILATED MACRO float mutual₂(float v₀, float v₁, float t /* ∈ [0, ⃨,1) */) { return v₀ + t * (v₁ - v₀); } /* H1: Compiles to 𝙼𝙰𝙳𝙳 and 𝙼𝚂𝚄𝙱. */
FOSSILATED MACRO float mutual₃(float v₀, float v₁, float t /* ∈ [0, ⃨,1) */) { return (1.0 - t) * v₀ + t * v₁; } /* H1: Compiles to 𝙼𝚄𝙻 and 𝙵𝙼𝙰𝙳𝙳. */
...
#pragma mark - Pumpin' 𝚏𝚕𝚘𝚊𝚝𝚜

simd_tᵦ ℯ🔭²(simd_tᵦ 𝒙); simd_tᵦ logₑ(simd_tᵦ 𝒙);
...
simd_tᵦ arctan(simd_tᵦ 𝒙); simd_t arctan(simd_t 𝒙);
simd_tᵦ floor(simd_tᵦ 𝒙); int32² nearest(simd_tᵦ 𝒙);
...
simd_tᵦ tanh⁻¹(simd_tᵦ 𝒙); simd_tᵦ tanh(simd_tᵦ 𝜃); /* -1 < tanh(𝜃) < 1 */
simd_tᵦ tan(simd_tᵦ 𝜃); /* -∞ < tan(𝜃) < ∞ */
simd_tᵦ atan₂(simd_tᵦ 𝒚, simd_tᵦ 𝒙); /* ≈ atan(𝒚/𝒙) */
 
#include <Additions/maths.hpp>
#include <Additions/math.hpp>

/* Default 𝚏𝚕𝚘𝚊𝚝 ∨ 𝚍𝚘𝚞𝚋𝚕𝚎 to integer conversion in the C language is 𝘛𝘳𝘶𝘯𝘤𝘢𝘵𝘦. */
...
typedef __builtin_uint_t Turtle; enum : Turtle {
  TurtleLeft        = 0b00000001, TurtleRight       = 0b00000010,
  TurtleForward     = 0b00000011, TurtleReverse     = 0b00000100,
  TurtleUp          = 0b00000101, TurtleDown        = 0b00000110,
...
typedef __builtin_uint_t RelationalStrider; enum :
   RelationalStrider { RelationalStriderAbaft = 
   0b00000001, RelationalStriderLeeward = 0b00000010,
