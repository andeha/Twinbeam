...
#include <Additions/math/simd².hpp> /* ⬷ later `#include-stochastic-when-debug`. */
...
simd_tᵦ pickᵦ(simd_tᵦ 𝗧, simd_tᵦ 𝗙, __uint128_t 𝗰𝗼𝗻𝗱ᵦ);
...
/* conditional reciprocal i․𝘦 $x ∨ x⁻¹$ where all signbits are sampled; a 
  '0' gives x and in-case '1' gives 1/x: */

MACRO simd_tᵦ reciprocalᵦ(simd_tᵦ 𝘹, __uint128_t 𝗶𝗻𝘃𝗲𝗿𝘁ᵦ) { simd_tᵦ 𝘹⁻¹ = 
  __builtin_simd_rcpᵦ(𝘹); return pickᵦ(𝘹⁻¹,𝘹,𝗶𝗻𝘃𝗲𝗿𝘁ᵦ); } /* x⁻¹ = 1/x */
MACRO simd_tᵦ negateᵦ(simd_tᵦ 𝘹, __uint128_t 𝗻𝗲𝗴ᵦ) { simd_tᵦ 🥇 ₊𝟶 = simd_initᵦ(+0.0); 
  simd_tᵦ ₋𝒙 = __builtin_simd_subᵦ(₊𝟶,𝘹); return pickᵦ(₋𝒙,𝘹,𝗻𝗲𝗴ᵦ); }
