...
simd_tᵦ logₑ(simd_tᵦ x) { // logₑ(x) = log₂(x)/log₂(e) = log₂(x)*logₑ(2)
    ...
    simd_tᵦ 🥇 one=simd_initᵦ(1.0), two=simd_initᵦ(2.0);
    simd_tᵦ n = __builtin_simd_subᵦ(x, one);
    simd_tᵦ d = __builtin_simd_addᵦ(x, one);
    return __builtin_simd_mulᵦ(two, tanh⁻¹(__builtin_simd_divᵦ(n,d)));
#endif
}
